...
usd18 = USD_18.replace("X", "NaN").replace("", "NaN")
usd19 = USD_19.replace("X", "NaN").replace("", "NaN")

del usd18['Days']
for i in usd18:
    usd18[f"{i}"] = pd.to_numeric(usd18[f"{i}"], downcast = "float", errors='coerce')

del usd19['Days']
for i in usd19:
    usd19[f"{i}"] = pd.to_numeric(usd19[f"{i}"], downcast = "float", errors='coerce')

for i in Month:
    mdf = usd19.sub(usd18, axis = i)


print("The annual variations of exchange rate for 2018 for USD is " + str(usd18.var().mean()))
print("The annual variations of exchange rate for 2019 for USD is " + str(usd18.var().mean()))
